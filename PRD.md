...
Version **0.1.0** ships as a Tauri v2 desktop app (macOS primary), consolidating NoteWeave / VoiceNotesAI / voiceforge / voiceweave product intent under GPL-3.0.
...
- AI organize, chat, RAG, agent, canvas, sync
...
|------|------------|--------|
| Capture | text, recording, scan, image, link, file, todo | NoteLoom |
| Dual capture | ⌘⇧T text, ⌘⇧V voice global shortcuts | NoteLoom v0.1 |
| Organize | Multi-select → templates → Markdown | NoteLoom + NoteWeave |
| Templates | Notes, Weekly Digest, Meeting Notes, Voice Dump | NoteLoom v0.1 |
| Editor | Tiptap MD (tables, tasks, code, math, Mermaid, outline, export) | NoteLoom |
| AI | Multi-provider chat/write/embed/OCR/STT/TTS | NoteLoom |
| Knowledge | Vector + hybrid RAG, memories, prompts | NoteLoom |
| Agent | Tool-approved agent over notes/records/files | NoteLoom |
| Extensibility | MCP servers, Skills | NoteLoom |
| Canvas | Flowcharts, mind maps, timelines, freehand, charts | NoteLoom |
| Sync | GitHub, GitLab, Gitee, Gitea, S3, WebDAV | NoteLoom |
